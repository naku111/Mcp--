 ...
 * @description 这是整个Web爬虫工具的“总指挥官”和入口点。
...
export class WebScraperMCPServer {
   ...
   */
  constructor() {
    // 1. 初始化MCP服务器，并设置其名称和版本。
    this.server = new Server({
      name: 'web-scraper-mcp',
      version: '1.0.0',
    });

    // 2. 实例化项目的所有核心模块，准备好被调用。
    this.scraper = new WebScraper();
    this.exportManager = new ExportManager();
    this.ruleEngine = new RuleEngine();
    this.headerManager = new HeaderManager();

    // 3. 设置工具的请求处理器，这是连接AI和我们代码的桥梁。
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          ...
          {
            name: 'scrape_url',
            description: '爬取单个指定URL的内容，能智能处理静态HTML和需要JavaScript渲染的动态页面（SPA）。',
            inputSchema: {
              type: 'object',
              properties: {
                url: { type: 'string', description: '【必需】要爬取的网页URL。' },
                format: { type: 'string', enum: ['markdown', 'text', 'html', 'json'], default: 'markdown', description: '期望返回内容的格式。' },
                usePuppeteer: { type: 'boolean', default: false, description: '是否强制使用Puppeteer无头浏览器。对于SPA页面或需要复杂交互的网站，应设为true。' },
                ruleSet: { type: 'string', description: '要应用的规则集名称，用于从页面中精准提取特定内容（如文章标题、正文等）。' },
                customHeaders: { type: 'object', description: '本次请求使用的一次性自定义请求头，可用于覆盖或补充全局设置。' }
              },
              required: ['url']
            }
          },
          // --- 工具2: create_rule_set ---
          {
            ...
            inputSchema: {
              type: 'object',
              properties: {
                name: { type: 'string', description: '【必需】规则集的唯一名称，例如 "my_blog_rules"。' },
                rules: { type: 'object', description: '【必需】具体的提取规则定义，使用CSS选择器。' }
              },
              required: ['name', 'rules']
            }
          },
          // --- 工具3: set_domain_headers ---
          {
            ...
            description: '为特定域名设置全局的自定义请求头（例如Cookie, Authorization），用于模拟登录状态或传递API密钥。',
            inputSchema: {
              type: 'object',
              properties: {
                domain: { type: 'string', description: '【必需】要设置请求头的域名，例如 "github.com"。' },
                headers: { type: 'object', description: '【必需】要设置的请求头键值对。' }
              },
          ...
          // --- 工具4: batch_scrape ---
          {
            ...
            inputSchema: {
              type: 'object',
              properties: {
                urls: { type: 'array', items: { type: 'string' }, description: '【必需】要爬取的URL列表。' },
  ...
  public async handleRequest(request: { name: string, arguments?: any }) {
    ...
    try {
      ...
      switch (name) {
        case 'scrape_url': return await this.handleScrapeUrl(args);
        case 'create_rule_set': return await this.handleCreateRuleSet(args);
        case 'set_domain_headers': return await this.handleSetDomainHeaders(args);
        case 'batch_scrape': return await this.handleBatchScrape(args);
        default: throw new Error(`未知或不支持的工具: ${name}`);
      }
    } catch (error) {
      // 捕获所有处理过程中可能发生的错误，并以标准的错误格式返回给AI。
      console.error(`处理工具 [${name}] 时发生错误:`, error);
      return { content: [{ type: 'text', text: `错误: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  }
  ...
  private async handleScrapeUrl(args: any) {
    const { url, format = 'markdown', usePuppeteer = false, ruleSet, customHeaders } = args;
    ...
    // 2. 执行爬取
    const content = await this.scraper.scrape(url, { usePuppeteer, headers: requestHeaders });
    ...
    // 4. 格式化并返回结果
    const exportedContent = await this.exportManager.export(processedContent, format);
  ...
  private async handleCreateRuleSet(args: any) {
    ...
    return { content: [{ type: 'text', text: `规则集 "${name}" 已成功创建并可供使用。` }] };
  }
  ...
  private async handleSetDomainHeaders(args: any) {
    ...
    return { content: [{ type: 'text', text: `域名 "${domain}" 的请求头已成功设置。` }] };
  }
  ...
  private async handleBatchScrape(args: any) {
    ...
    const scrapePromises = urls.map(async (url: string) => {
      try {
        ...
        const headers = isDataUrl ? {} : this.headerManager.getHeaders(new URL(url).hostname);
        const content = await this.scraper.scrape(url, { usePuppeteer, headers });
        let processedContent = content;
        if (ruleSet) processedContent = await this.ruleEngine.applyRules(ruleSet, content);
        const exportedContent = await this.exportManager.export(processedContent, format);
    ...
    const results = settledResults.map(result => {
      ...
      // 如果任务失败，记录失败原因。
      return { success: false, error: `一个未知的爬取任务失败: ${result.reason}` };
    });
