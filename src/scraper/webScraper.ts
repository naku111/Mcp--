  ...
  public static async ensureBrowserIsReady(): Promise<string> {
    ...
    for (const path of defaultChromePaths) {
      if (fs.existsSync(path)) {
        console.log(`[浏览器管理器] 成功找到系统安装的Chrome: ${path}`);
    ...
    console.log('[浏览器管理器] 未在常规路径找到Chrome，将尝试使用Puppeteer下载的版本。');
    ...
    try {
      ...
      if (execPath && fs.existsSync(execPath)) {
        console.log(`[浏览器管理器] 成功定位到Puppeteer内置的浏览器: ${execPath}`);
...
export class WebScraper {
  ...
  /**
   * 智能爬取入口方法。
   * 这是一个策略分发器，根据URL类型、用户选项和页面内容特征，动态选择最优的爬取策略。
   * @param url 要爬取的URL
   * @param options 爬取选项
   */
  async scrape(url: string, options: ScrapeOptions = {}): Promise<ScrapedContent> {
    ...
    // 策略1：如果URL是data URI，直接使用Puppeteer处理
    if (url.startsWith('data:')) {
      console.log('检测到 data: URI, 直接使用 Puppeteer 进行处理...');
      return this.scrapeWithPuppeteer(url, options);
    }

    // 策略2：如果用户强制指定，直接使用Puppeteer
    if (usePuppeteer) {
      console.log('用户强制指定使用 Puppeteer 进行爬取...');
      return this.scrapeWithPuppeteer(url, options);
    }

    // 策略3（默认）：先尝试快速的静态爬取
    console.log(`[快速模式] 正在对 ${url} 进行静态爬取...`);
    try {
      const staticContent = await this.scrapeWithAxios(url, options);
      
      // 对静态内容进行启发式分析，判断是否为需要JS渲染的SPA页面
      const jsRequiredKeywords = [
        '您需要允许该网站执行 JavaScript', 'enable JavaScript', 'requires JavaScript',
        'not support a browser that has JavaScript disabled', 'Please enable JavaScript'
      ];
      ...
      if (contentIsJSReliant) {
        console.warn('警告：静态内容可能不完整（依赖JavaScript），自动切换到Puppeteer深度模式...');
        return this.scrapeWithPuppeteer(url, options);
      }

      console.log('[快速模式] 静态爬取成功。');
      return staticContent;
    } catch (error) {
      // 如果静态爬取失败（如网络错误、超时），则自动降级到更强大的Puppeteer模式重试
      console.error(`静态爬取失败: ${error instanceof Error ? error.message : '未知错误'}。自动尝试使用 Puppeteer...`);
      return this.scrapeWithPuppeteer(url, options);
    }
  }
   ...
   * 适用于传统的、非SPA的服务端渲染页面。
   */
  private async scrapeWithAxios(url: string, options: ScrapeOptions): Promise<ScrapedContent> {
    ...
    try {
      ...
      // 处理返回数据，确保是字符串格式
      const htmlContent = typeof response.data === 'object' ? JSON.stringify(response.data) : String(response.data);
      const $ = cheerio.load(htmlContent);
      return this.extractContent($, url, htmlContent);
    } catch (error) {
      throw new Error(`Axios爬取失败: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async scrapeWithPuppeteer(url: string, options: ScrapeOptions): Promise<ScrapedContent> {
    const { headers = {}, timeout = 60000, waitForSelector, screenshot = false } = options;
    ...
    try {
      ...
      console.log(`[Puppeteer] 正在导航到: ${url} (超时: ${timeout / 1000}s)`);
      await page.goto(url, { waitUntil: 'domcontentloaded', timeout });
      console.log('[Puppeteer] 页面DOM加载完成，开始智能等待动态内容...');
      ...
      if (waitForSelector) {
        ...
        console.log('[Puppeteer] 检测到今日头条，特殊等待策略启动...');
        try {
          await page.waitForSelector('div.feed-card, div.article-content', { timeout: 15000 });
          console.log('[Puppeteer] 今日头条特定内容加载成功。');
        } catch (e) { console.warn('[Puppeteer] 等待今日头条元素超时，将继续执行...'); }
      } else {
      ...
      if (screenshot) await page.screenshot({ path: `screenshot-${Date.now()}.png` });
      
      const $ = cheerio.load(html);
      return this.extractContent($, url, html);
    } catch (error) {
      throw new Error(`Puppeteer爬取失败: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
   ...
   * 这是一个通用的内容解析器。
   */
  private extractContent($: cheerio.CheerioAPI, url: string, html: string): ScrapedContent {
    const title = $('title').text().trim() || $('h1').first().text().trim() || '无标题';
    let content = '';
    
    // 优先级列表，用于寻找最可能包含主要内容的容器
    const contentSelectors = ['main', 'article', '.content', '.post-content', '.entry-content', '#content', '.main-content', 'body'];

    for (const selector of contentSelectors) {
      const element = $(selector);
      if (element.length > 0) {
        // 移除脚本、样式、导航等无关元素
        element.find('script, style, nav, header, footer, aside, .actions, .buttons').remove();
        content = element.text().trim();
        // 如果内容足够长，就认为找到了主要内容，停止搜索
    ...
    return {
      url, title, content: this.cleanText(content), html,
